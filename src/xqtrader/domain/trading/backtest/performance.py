...
def print_performance(perf: dict, strategy_name: str = "") -> None:
    """打印绩效指标"""
...
def print_trade_records(strat: XqTraderStrategy) -> None:
    ...
    for r in records:
        ...
        rows.append({
            "序号": trade_idx,
            "日期": str(r["date"]),
            "方向": "买入" if r["direction"] == "buy" else "卖出",
            "价格": f"{r['price']:.2f}",
            "数量": r["quantity"],
            "信号依据": r["signal_reason"],
        })
