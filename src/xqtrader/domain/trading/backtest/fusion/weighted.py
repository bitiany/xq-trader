...
def _direction_signal(direction: str) -> float:
    """将方向转换为数值信号: buy/bullish→+1, sell/bearish→-1, neutral→0"""
...
def _positive_direction(results: list[RuleResult]) -> str:
    if any(r.direction == "bullish" for r in results):
        return "bullish"
    return "buy"


def _negative_direction(results: list[RuleResult]) -> str:
    if any(r.direction == "bearish" for r in results):
        return "bearish"
    return "sell"
    ...
    def fuse(self, results: list[RuleResult], config: FusionConfig) -> RuleResult:
        ...
        if normalized >= config.buy_threshold:
            return RuleResult(
                ...
                direction=_positive_direction(results),
        ...
        if normalized <= -config.sell_threshold:
            return RuleResult(
                rule_id="fusion", passed=True,
                score=abs(normalized),
                direction=_negative_direction(results),
                confidence=abs(normalized),
