"""A股财务指标采集任务（Tushare fina_indicator 数据源）。

数据源：Tushare fina_indicator 接口（doc_id=79），采集上市公司财务指标数据写入 FinancialIndicator 表。
管线流程（每个标的串行执行）：
  WatermarkAspect(前切) → DownloadStage → CleanStage → PersistStage → WatermarkAspect(后切)

字段映射（fina_indicator → FinancialIndicator）：
  ts_code → symbol
  ann_date → ann_date
  end_date → end_date
  其余字段同名
...
注意事项：
  - fina_indicator 接口单次最多返回 100 条，需按标的逐个采集
  - 显式指定 fields 参数，确保全部字段（含默认不显示字段）均被采集
  - 财务指标为季度数据，水位基于 ann_date（公告日期），非交易日维度
"""
...
_DATA_TYPE = "financial_indicator"

# fina_indicator → FinancialIndicator 列映射
_COLUMN_MAPPING: dict[str, str] = {
    "ts_code": "symbol",
}

# ORM 模型中存在的全部列名（用于过滤 Tushare 返回的额外字段）
# 从 FinancialIndicator ORM 模型自动推导，无需手动维护
_ORM_COLUMNS: set[str] = {c.name for c in FinancialIndicator.__table__.columns}

# 数值列（需要强制转 numeric + 精度处理）— 仅 ORM 中存在的列
_NUMERIC_COLS = [c for c in _ORM_COLUMNS if c not in {"symbol", "ann_date", "end_date", "update_flag"}]
...
def clean_fina_indicator_data(df: pd.DataFrame) -> pd.DataFrame:
    """财务指标数据清洗 — 列映射 + 格式统一 + 数值处理。

    1. 列名映射（fina_indicator → FinancialIndicator）
    2. 过滤掉 ORM 模型中不存在的列（Tushare 返回的额外字段）
    3. ann_date/end_date 格式统一 YYYY-MM-DD
    ...
    if extra_cols:
        logger.debug("[fina_indicator.clean] 过滤 ORM 外字段: %s", extra_cols)
...
async def persist_fina_indicator_data(df: pd.DataFrame) -> int:
    """将财务指标数据 upsert 到 FinancialIndicator 表。"""
    instances = DataFrameToModelConverter.convert(
        df=df,
        model_class=FinancialIndicator,
        custom_transforms=_PERSIST_CUSTOM_TRANSFORMS,
    )
    if not instances:
        return 0
    return await FinancialIndicator.bulk_create_or_update(
...
class FinancialIndicatorError(PipelineError):
    """财务指标采集异常基类。"""


class DownloadError(FinancialIndicatorError):
    """下载阶段异常。"""


class PersistError(FinancialIndicatorError):
    """持久化阶段异常。"""


class DownloadStage(Stage):
    """下载阶段 — 调用 TushareDataCollector 获取财务指标数据。"""
    ...
    async def process(self, item: Any, ctx: PipelineContext) -> StageResult:
        ...
        try:
            ...
            df = await _get_collector().fetch_fina_indicator(
                ts_code=stock_code,
                start_date=ts_start,
                end_date=ts_end,
            )
            ...
            if df is None or df.empty:
                logger.debug(
                    "[fina_indicator.collect] 无数据: %s range=%s~%s",
...
class CleanStage(Stage):
    ...
    async def process(self, item: Any, ctx: PipelineContext) -> StageResult:
        ...
        initial_len = len(df)
        df = clean_fina_indicator_data(df)
        ...
        if cleaned > 0:
            logger.debug(
                "[fina_indicator.collect] 清洗: %s removed %d invalid rows",
                stock_code, cleaned,
            )
...
class PersistStage(Stage):
    """持久化阶段 — 将财务指标数据写入 FinancialIndicator 表。"""
    ...
    async def process(self, item: Any, ctx: PipelineContext) -> StageResult:
        ...
        try:
            count = await persist_fina_indicator_data(df)
...
class FinancialIndicatorCollectTask(BaseTask):
    """A股财务指标采集任务（Tushare fina_indicator 数据源）。

    入参：
      - data_type: 数据类型（默认 financial_indicator）
      - concurrency: 并发数（默认 3，fina_indicator 接口限流较严）
      - stock_codes: 股票代码列表（为空时采集全市场）
      - max_count: 最大标的数量（用于测试，0 表示不限）
      - collect_date: 采集起始日期（格式 YYYY-MM-DD，为空时按水位日期增量采集）
    """

    task_name = "market.financial_indicator_collect"
    description = "A股财务指标采集-Tushare fina_indicator数据源（管道引擎并发）"
    ...
    async def _run_impl(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if not stock_codes:
            ...
            if not stock_codes:
                logger.warning("[fina_indicator.collect] 未找到任何标的代码")
                return {"total": 0, "succeeded": 0, "failed": 0}

        # 限制标的数量（用于测试）
        if max_count > 0 and len(stock_codes) > max_count:
            stock_codes = stock_codes[:max_count]
            logger.debug("[fina_indicator.collect] 限制标的数量: max_count=%d", max_count)
        ...
        logger.info(
            "[fina_indicator.collect] 开始采集: pipeline=%s concurrency=%d stocks=%d collect_date=%s",
    ...
    async def _get_all_stock_codes() -> list[str]:
        ...
        logger.debug("[fina_indicator.collect] 全市场标的数: %d", len(codes))
        return codes
