...
1. **明确意图**：用户找因子？查值？看评估？
2. **若找因子**：先 `mcp_xq_factors_xq_list_factor_categories` 显示分类，再 `mcp_xq_factors_xq_list_factors` 用 `keyword` 过滤。
3. **若查因子值**：必须传 `trade_date`；如未指定 `pool_id` 默认用 `all`；symbol 可选。
4. **若看评估**：用 `mcp_xq_factors_xq_get_factor_stats_latest`，结合 IC、ICIR、分层收益给出解读。
